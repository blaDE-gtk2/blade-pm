...
static void blpm_unique_dbus_class_init 	(XfpmUniqueClass *klass);
static void blpm_unique_dbus_init		(XfpmUnique *unique);

static void blpm_unique_finalize   		(GObject *object);
...
G_DEFINE_TYPE (XfpmUnique, blpm_unique, G_TYPE_OBJECT)

static void blpm_unique_get_property (GObject *object,
				      guint prop_id,
                                      GValue *value,
                                      GParamSpec *pspec)
{
...
static void blpm_unique_set_property (GObject *object,
				      guint prop_id,
                                      const GValue *value,
                                      GParamSpec *pspec)
{
...
static void
blpm_unique_class_init (XfpmUniqueClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->finalize = blpm_unique_finalize;
    
    object_class->set_property = blpm_unique_set_property;
    object_class->get_property = blpm_unique_get_property;
    ...
    blpm_unique_dbus_class_init (klass);
}

static void
blpm_unique_init (XfpmUnique *unique)
{
    GError *error = NULL;
    ...
    blpm_unique_dbus_init (unique);
}

static void
blpm_unique_finalize (GObject *object)
{
    XfpmUnique *unique;

    unique = XFPM_UNIQUE (object);
    
    blpm_dbus_release_name (dbus_g_connection_get_connection (unique->priv->bus),
			    unique->priv->name);
    
    dbus_g_connection_unref (unique->priv->bus);
    g_free (unique->priv->name);

    G_OBJECT_CLASS (blpm_unique_parent_class)->finalize (object);
}

XfpmUnique *
blpm_unique_new (const gchar *name)
{
...
gboolean blpm_unique_app_is_running (XfpmUnique *unique)
{
    g_return_val_if_fail (XFPM_IS_UNIQUE (unique), FALSE);
    
    if (blpm_dbus_name_has_owner (dbus_g_connection_get_connection (unique->priv->bus),
				  unique->priv->name))
    {
	DBusGProxy *proxy;
	GError *error = NULL;
    ...
    blpm_dbus_register_name (dbus_g_connection_get_connection (unique->priv->bus),
			     unique->priv->name);

...
#include "blpm-dbus-marshal.h"

static void blpm_dbus_monitor_finalize   (GObject *object);
...
G_DEFINE_TYPE (XfpmDBusMonitor, blpm_dbus_monitor, G_TYPE_OBJECT)

static void
blpm_dbus_monitor_free_watch_data (XfpmWatchData *data)
{
    g_free (data->name);
    g_free (data);
}
...
blpm_dbus_monitor_get_watch_data (GPtrArray *array, const gchar *name, DBusBusType bus_type)
{
    XfpmWatchData *data;
    guint i;
...
static void
blpm_dbus_monitor_unique_connection_name_lost (XfpmDBusMonitor *monitor, DBusBusType bus_type, const gchar *name)
{
    XfpmWatchData *watch;
    guint i = 0;
	...
	if ( !g_strcmp0 (watch->name, name) && bus_type == watch->bus_type )
	{
	    ...
	    blpm_dbus_monitor_free_watch_data (watch);
	}
    }
}

static void
blpm_dbus_monitor_service_connection_changed (XfpmDBusMonitor *monitor, DBusBusType bus_type, 
...
static void
blpm_dbus_monitor_name_owner_changed (XfpmDBusMonitor *monitor, const gchar *name,
				      const gchar *prev, const gchar *new, DBusBusType bus_type)
{
    if ( strlen (prev) != 0 )
    {
	blpm_dbus_monitor_unique_connection_name_lost (monitor, bus_type, prev);
	
	/* Connection has name */
	if ( strlen (name) != 0 )
	    blpm_dbus_monitor_service_connection_changed (monitor, bus_type, name, FALSE);
    }
    else if ( strlen (name) != 0 && strlen (new) != 0)
    {
	blpm_dbus_monitor_service_connection_changed (monitor, bus_type, name, TRUE);
    }
}

static void
blpm_dbus_monitor_session_name_owner_changed_cb (DBusGProxy *proxy, const gchar *name,
						 const gchar *prev, const gchar *new,
						 XfpmDBusMonitor *monitor)
{
    blpm_dbus_monitor_name_owner_changed (monitor, name, prev, new, DBUS_BUS_SESSION);
}

static void
blpm_dbus_monitor_system_name_owner_changed_cb  (DBusGProxy *proxy, const gchar *name,
						 const gchar *prev, const gchar *new,
						 XfpmDBusMonitor *monitor)
{
    blpm_dbus_monitor_name_owner_changed (monitor, name, prev, new, DBUS_BUS_SYSTEM);
}

static gboolean
blpm_dbus_monitor_query_system_bus_idle (gpointer data)
{
    XfpmDBusMonitor *monitor;
    DBusGConnection *bus;
    GError *error = NULL;
...
static void
blpm_dbus_monitor_setup_system_watch (XfpmDBusMonitor *monitor)
{
    g_timeout_add_seconds (2, (GSourceFunc) blpm_dbus_monitor_query_system_bus_idle, monitor);
}

static DBusHandlerResult
blpm_dbus_monitor_system_bus_filter (DBusConnection *bus, DBusMessage *message, void *data)
{
    XfpmDBusMonitor *monitor;
	...
	blpm_dbus_monitor_setup_system_watch (monitor);
...
static void
blpm_dbus_monitor_session (XfpmDBusMonitor *monitor)
{
    ...
    dbus_g_proxy_connect_signal (monitor->priv->session_proxy, "NameOwnerChanged",
				 G_CALLBACK (blpm_dbus_monitor_session_name_owner_changed_cb), monitor, NULL);
}

static void
blpm_dbus_monitor_system (XfpmDBusMonitor *monitor)
{
    ...
    dbus_g_proxy_connect_signal (monitor->priv->system_proxy, "NameOwnerChanged",
				 G_CALLBACK (blpm_dbus_monitor_system_name_owner_changed_cb), monitor, NULL);
}

static void
blpm_dbus_monitor_class_init (XfpmDBusMonitorClass *klass)
{
    ...
    signals [UNIQUE_NAME_LOST] =
    	g_signal_new ("unique-name-lost",
		      ...
		      NULL, NULL,
		      _blpm_dbus_marshal_VOID__STRING_BOOLEAN,
		      G_TYPE_NONE, 2, 
    ...
    signals [SERVICE_CONNECTION_CHANGED] =
    	g_signal_new ("service-connection-changed",
		      ...
		      NULL, NULL,
		      _blpm_dbus_marshal_VOID__STRING_BOOLEAN_BOOLEAN,
    ...
    object_class->finalize = blpm_dbus_monitor_finalize;

    g_type_class_add_private (klass, sizeof (XfpmDBusMonitorPrivate));
}

static void
blpm_dbus_monitor_init (XfpmDBusMonitor *monitor)
{
    ...
    blpm_dbus_monitor_session (monitor);
    blpm_dbus_monitor_system  (monitor);
    ...
    dbus_connection_add_filter (dbus_g_connection_get_connection (monitor->priv->system_bus),
			        blpm_dbus_monitor_system_bus_filter,
				monitor, 
				NULL);
}

static void
blpm_dbus_monitor_finalize (GObject *object)
{
    XfpmDBusMonitor *monitor;
	...
	dbus_g_proxy_disconnect_signal (monitor->priv->session_proxy, "NameOwnerChanged",
					G_CALLBACK (blpm_dbus_monitor_session_name_owner_changed_cb), monitor);
	...
	dbus_g_proxy_disconnect_signal (monitor->priv->system_proxy, "NameOwnerChanged",
				        G_CALLBACK (blpm_dbus_monitor_system_name_owner_changed_cb), monitor);
    ...
    g_ptr_array_foreach (monitor->priv->names_array, (GFunc) blpm_dbus_monitor_free_watch_data, NULL);
    g_ptr_array_foreach (monitor->priv->services_array, (GFunc) blpm_dbus_monitor_free_watch_data, NULL);

    g_ptr_array_free (monitor->priv->names_array, TRUE);
    g_ptr_array_free (monitor->priv->services_array, TRUE);

    G_OBJECT_CLASS (blpm_dbus_monitor_parent_class)->finalize (object);
}

XfpmDBusMonitor *
blpm_dbus_monitor_new (void)
{
    static gpointer blpm_dbus_monitor_object = NULL;
    
    if ( G_LIKELY (blpm_dbus_monitor_object != NULL) )
    {
	g_object_ref (blpm_dbus_monitor_object);
    }
    else
    {
	blpm_dbus_monitor_object = g_object_new (XFPM_TYPE_DBUS_MONITOR, NULL);
	g_object_add_weak_pointer (blpm_dbus_monitor_object, &blpm_dbus_monitor_object);
    }
    
    return XFPM_DBUS_MONITOR (blpm_dbus_monitor_object);
}

gboolean blpm_dbus_monitor_add_unique_name (XfpmDBusMonitor *monitor, DBusBusType bus_type, const gchar *unique_name)
{
    XfpmWatchData *watch;
    ...
    /* We have it already */
    if ( blpm_dbus_monitor_get_watch_data (monitor->priv->names_array, unique_name, bus_type) )
	return FALSE;
...
void blpm_dbus_monitor_remove_unique_name (XfpmDBusMonitor *monitor, DBusBusType bus_type, const gchar *unique_name)
{
    XfpmWatchData *watch;
    
    g_return_if_fail (XFPM_IS_DBUS_MONITOR (monitor));

    watch = blpm_dbus_monitor_get_watch_data (monitor->priv->names_array, unique_name, bus_type);
...
gboolean blpm_dbus_monitor_add_service (XfpmDBusMonitor *monitor, DBusBusType bus_type, const gchar *service_name)
{
    XfpmWatchData *watch;
    
    g_return_val_if_fail (XFPM_IS_DBUS_MONITOR (monitor), FALSE);
    
    if ( blpm_dbus_monitor_get_watch_data (monitor->priv->services_array, service_name, bus_type ) )
	return FALSE;
...
void blpm_dbus_monitor_remove_service (XfpmDBusMonitor *monitor, DBusBusType bus_type, const gchar *service_name)
{
    XfpmWatchData *watch;
    
    g_return_if_fail (XFPM_IS_DBUS_MONITOR (monitor));
    
    watch = blpm_dbus_monitor_get_watch_data (monitor->priv->services_array, service_name, bus_type);

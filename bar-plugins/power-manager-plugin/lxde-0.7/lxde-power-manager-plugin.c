...
/* prototypes */
static GtkWidget * power_manager_plugin_construct (LXBar *bar, config_setting_t *settings);

/* register the plugin */
FM_DEFINE_MODULE(lxbar_gtk, power_manager_plugin);

/* Plugin descriptor. */
LXBarPluginInit fm_module_init_lxbar_gtk = {
...
static GtkWidget *
power_manager_plugin_new (LXBar *bar)
{
    ...
    lxbar_plugin_set_data(power_manager_plugin->ebox, power_manager_plugin, g_free);
...
static GtkWidget *
power_manager_plugin_construct (LXBar *bar, config_setting_t *settings)
{
    /* create the plugin */
    return power_manager_plugin_new (bar);
}

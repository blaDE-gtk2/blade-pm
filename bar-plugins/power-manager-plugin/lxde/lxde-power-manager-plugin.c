...
PluginClass lxde_power_manager_plugin_class = {
    ...
    config : NULL,
    save : NULL,
    bar_configuration_changed : lxde_power_manager_plugin_configuration_changed
};
...
lxde_power_manager_plugin_configuration_changed(Plugin *p)
{
    ...
    GtkOrientation orientation = (p->bar->orientation == GTK_ORIENTATION_VERTICAL) ? GTK_ORIENTATION_VERTICAL : GTK_ORIENTATION_HORIZONTAL;

    int size = (orientation == GTK_ORIENTATION_VERTICAL) ? p->bar->width : p->bar->height;
    ...
    power_manager_button_set_width (POWER_MANAGER_BUTTON(power_manager_plugin->power_manager_button), p->bar->icon_size);
}

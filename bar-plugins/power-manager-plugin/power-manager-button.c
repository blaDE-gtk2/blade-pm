...
#include "common/blpm-config.h"
#include "common/blpm-icons.h"
#include "common/blpm-power-common.h"
#include "common/blpm-brightness.h"
...
struct PowerManagerButtonPrivate
{
    ...
    /* The actual bar icon image */
    GtkWidget       *bar_icon_image;
     ...
     * bar image and tooltip description */
...
power_manager_button_update_device_icon_and_details (PowerManagerButton *button, UpDevice *device)
{
	...
	/* it is! update the bar button */
	g_free(button->priv->bar_icon_name);
	button->priv->bar_icon_name = icon_name;
...
brightness_up (PowerManagerButton *button)
{
    ...
    blpm_brightness_get_level (button->priv->brightness, &level);
    max_level = blpm_brightness_get_max_level (button->priv->brightness);
...
power_manager_button_scroll_event (GtkWidget *widget, GdkEventScroll *ev)
{
    ...
    hw_found = blpm_brightness_has_hw (button->priv->brightness);
...
{
    gint32 max_level = blpm_brightness_get_max_level (button->priv->brightness);
...
power_manager_button_init (PowerManagerButton *button)
{
    ...
    button->priv->brightness = blpm_brightness_new ();
    blpm_brightness_setup (button->priv->brightness);
    ...
    {
        button->priv->channel = blconf_channel_get ("blade-pm");
    }

    /* Sane defaults for the bar icon */
    button->priv->bar_icon_name = g_strdup(XFPM_AC_ADAPTER_ICON);
    button->priv->bar_icon_width = 24;
...
GtkWidget *
#ifdef XFCE_PLUGIN
power_manager_button_new (BladeBarPlugin *plugin)
#endif
#ifdef LXDE_PLUGIN
...
#ifdef XFCE_PLUGIN
    button->priv->plugin = BLADE_BAR_PLUGIN (g_object_ref (plugin));
#endif

    blconf_g_property_bind(button->priv->channel,
...
power_manager_button_set_icon (PowerManagerButton *button)
{
    ...
    DBG("icon_width %d", button->priv->bar_icon_width);

    pixbuf = gtk_icon_theme_load_icon (gtk_icon_theme_get_default (),
                                       button->priv->bar_icon_name,
                                       button->priv->bar_icon_width,
                                       GTK_ICON_LOOKUP_GENERIC_FALLBACK,
                                       NULL);

    if ( pixbuf )
    {
        gtk_image_set_from_pixbuf (GTK_IMAGE (button->priv->bar_icon_image), pixbuf);
...
{
    return button->priv->bar_icon_name;
}
...
power_manager_button_set_width (PowerManagerButton *button, gint width)
{
    ...
    button->priv->bar_icon_width = width;
...
#ifdef XFCE_PLUGIN
static gboolean
power_manager_button_size_changed_cb (BladeBarPlugin *plugin, gint size, PowerManagerButton *button)
{
    gint width;
    gint xthickness;
    gint ythickness;
    ...
    size /= blade_bar_plugin_get_nrows (plugin);
...
static void
power_manager_button_style_set_cb (BladeBarPlugin *plugin, GtkStyle *prev_style, PowerManagerButton *button)
{
    gtk_widget_reset_rc_styles (GTK_WIDGET (plugin));
    power_manager_button_size_changed_cb (plugin, blade_bar_plugin_get_size (plugin), button);
}

static void
power_manager_button_free_data_cb (BladeBarPlugin *plugin, PowerManagerButton *button)
{
...
#ifdef XFCE_PLUGIN
    blade_bar_plugin_add_action_widget (button->priv->plugin, GTK_WIDGET (button));
    blade_bar_plugin_set_small (button->priv->plugin, TRUE);
#endif

    button->priv->bar_icon_image = gtk_image_new ();
    gtk_container_add (GTK_CONTAINER (button), button->priv->bar_icon_image);
...
#ifdef XFCE_PLUGIN
    blade_bar_plugin_menu_insert_item (button->priv->plugin, GTK_MENU_ITEM (mi));
...
menu_item_activate_cb(GtkWidget *object, gpointer user_data)
{
    ...
    for (item = g_list_first (button->priv->devices); item != NULL; item = g_list_next (item))
    {
        ...
        if (battery_device->menu_item == object)
        {
            /* Call blpm settings with the device id */
            blpm_preferences_device_id (battery_device->object_path);
            return;
        }
    }
}
...
power_manager_button_menu_add_device (PowerManagerButton *button, BatteryDevice *battery_device, gboolean append)
{
    ...
    /* Active calls blpm settings with the device's id to display details */
...
decrease_brightness (PowerManagerButton *button)
{
    ...
    if ( !blpm_brightness_has_hw (button->priv->brightness) )
        return;
    ...
    if ( level > button->priv->brightness_min_level )
    {
        blpm_brightness_down (button->priv->brightness, &level);
        if (button->priv->range)
...
increase_brightness (PowerManagerButton *button)
{
    ...
    if ( level < max_level )
    {
        blpm_brightness_up (button->priv->brightness, &level);
        if (button->priv->range)
            gtk_range_set_value (GTK_RANGE (button->priv->range), level);
    }
}
...
brightness_set_level_with_timeout (PowerManagerButton *button)
{
    ...
    blpm_brightness_get_level (button->priv->brightness, &hw_level);

    if ( hw_level != range_level )
    {
        blpm_brightness_set_level (button->priv->brightness, range_level);
    }
...
power_manager_button_show_menu (PowerManagerButton *button)
{
    ...
    if ( blpm_brightness_has_hw (button->priv->brightness) )
    {
        ...
        blpm_brightness_get_level (button->priv->brightness, &current_level);
    ...
    gtk_widget_show(mi);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), mi);
    g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(blpm_preferences), NULL);

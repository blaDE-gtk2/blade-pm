...
#include "interfaces/blpm-settings_ui.h"
...
void        blpm_update_logind_handle_lid_switch   (BlconfChannel *channel);
/* END Light Locker Integration */

void brightness_level_on_ac (GtkWidget *w,  BlconfChannel *channel)
{
    guint val = (guint) gtk_range_get_value (GTK_RANGE (w));
    
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_AC, val) )
    {
	g_critical ("Unable to set value %u for property %s\n", val, BRIGHTNESS_LEVEL_ON_AC);
    }
}

void brightness_level_on_battery (GtkWidget *w,  BlconfChannel *channel)
{
     guint val = (guint) gtk_range_get_value (GTK_RANGE (w));
    
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_BATTERY, val) )
    {
...
void
battery_critical_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX CRITICAL_BATT_ACTION_CFG, value) )
    {
...
void
inactivity_on_ac_value_changed_cb (GtkWidget *widget, BlconfChannel *channel)
{
    gint value    = (gint)gtk_range_get_value (GTK_RANGE (widget));
    
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX ON_AC_INACTIVITY_TIMEOUT, value))
    {
...
void
inactivity_on_battery_value_changed_cb (GtkWidget *widget, BlconfChannel *channel)
{
    gint value    = (gint)gtk_range_get_value (GTK_RANGE (widget));
    
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX ON_BATTERY_INACTIVITY_TIMEOUT, value))
    {
...
void
button_sleep_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    GtkTreeIter       selected_row;
    gint value = 0;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX SLEEP_SWITCH_CFG, value ) )
    {
	g_critical ("Cannot set value for property %s\n", SLEEP_SWITCH_CFG);
    }
}
...
void
button_power_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    GtkTreeIter       selected_row;
    gint value = 0;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX POWER_SWITCH_CFG, value) )
    {
	g_critical ("Cannot set value for property %s\n", POWER_SWITCH_CFG);
    }
}
...
void
button_hibernate_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    GtkTreeIter       selected_row;
    gint value = 0;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX HIBERNATE_SWITCH_CFG, value ) )
    {
...
void
notify_toggled_cb (GtkWidget *w, BlconfChannel *channel)
{
    gboolean val = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(w));
    
    if (!blconf_channel_set_bool (channel, PROPERTIES_PREFIX GENERAL_NOTIFICATION_CFG, val) )
    {
...
void
systray_toggled_cb (GtkWidget *w, BlconfChannel *channel)
{
    gboolean val = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(w));

    if (!blconf_channel_set_int (channel, PROPERTIES_PREFIX SHOW_TRAY_ICON_CFG, (int)val) )
    {
	g_critical ("Cannot set value for property %s\n", SHOW_TRAY_ICON_CFG);
    }
}
...
void
on_ac_sleep_mode_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    GtkTreeIter       selected_row;
    guint value = 0;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX INACTIVITY_SLEEP_MODE_ON_AC, value) )
    {
...
void
on_battery_sleep_mode_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    GtkTreeIter       selected_row;
    guint value = 0;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX INACTIVITY_SLEEP_MODE_ON_BATTERY, value) )
    {
...
void
dpms_toggled_cb (GtkWidget *w, BlconfChannel *channel)
{
    gboolean val = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(w));
    
    blconf_channel_set_bool (channel, PROPERTIES_PREFIX DPMS_ENABLED_CFG, val);
...
void
display_blank_on_battery_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkWidget *brg;
...
void
sleep_on_battery_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkWidget *brg;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX ON_BATT_DPMS_SLEEP, sleep_value))
    {
	g_critical ("Cannot set value for property %s\n", ON_BATT_DPMS_SLEEP);
    }
}
...
void
off_on_battery_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX ON_BATT_DPMS_OFF, off_value))
    {
	g_critical ("Cannot set value for property %s\n", ON_BATT_DPMS_OFF);
    }
}
...
void
display_blank_on_ac_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkWidget *brg;
...
void
sleep_on_ac_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkWidget *brg;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX ON_AC_DPMS_SLEEP, sleep_value))
    {
	g_critical ("Cannot set value for property %s\n", ON_AC_DPMS_SLEEP);
    }
}
...
void
off_on_ac_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    gint off_value    = (gint)gtk_range_get_value (GTK_RANGE(w));
    gint sleep_value  = (gint)gtk_range_get_value (GTK_RANGE(on_ac_dpms_sleep));
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX ON_AC_DPMS_OFF, off_value))
    {
	g_critical ("Cannot set value for property %s\n", ON_AC_DPMS_OFF);
    }
}
...
void
brightness_handle_keys_toggled_cb (GtkWidget *w, BlconfChannel *channel)
{
    gboolean val = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(w));

    if ( !blconf_channel_set_bool (channel, PROPERTIES_PREFIX HANDLE_BRIGHTNESS_KEYS, val) )
    {
        g_critical ("Cannot set value for property %s\n", HANDLE_BRIGHTNESS_KEYS);
    }
    if ( !blconf_channel_set_int (channel, PROPERTIES_PREFIX BRIGHTNESS_SWITCH, !val) )
    {
        g_critical ("Cannot set value for property %s\n", BRIGHTNESS_SWITCH);
    }
}
...
void
brightness_on_battery_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_ON_BATTERY, value))
    {
...
void
brightness_on_ac_value_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    gint value    = (gint)gtk_range_get_value (GTK_RANGE (w));
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_ON_AC, value))
    {
	g_critical ("Cannot set value for property %s\n", BRIGHTNESS_ON_AC);
    }
}
...
void
on_battery_lid_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    GtkTreeIter       selected_row;
    gint value = 0;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX LID_SWITCH_ON_BATTERY_CFG, value) )
    {
	g_critical ("Cannot set value for property %s\n", LID_SWITCH_ON_BATTERY_CFG);
    }
    blpm_update_logind_handle_lid_switch (channel);
}

void
on_ac_lid_changed_cb (GtkWidget *w, BlconfChannel *channel)
{
    GtkTreeModel     *model;
    GtkTreeIter       selected_row;
    gint value = 0;
    ...
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX LID_SWITCH_ON_AC_CFG, value) )
    {
...
void
critical_level_value_changed_cb (GtkSpinButton *w, BlconfChannel *channel)
{
    guint val = (guint) gtk_spin_button_get_value (w);
    
    if (!blconf_channel_set_uint (channel, PROPERTIES_PREFIX CRITICAL_POWER_LEVEL, val) )
    {
...
void
lock_screen_toggled_cb (GtkWidget *w, BlconfChannel *channel)
{
    BlconfChannel *session_channel = blconf_channel_get ("xfce4-session");
    gboolean val = (gint) gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(w));

    if ( !blconf_channel_set_bool (session_channel, "/shutdown/LockScreen", val) )
    {
	g_critical ("Unable to set value for property %s\n", LOCK_SCREEN_ON_SLEEP);
    }

    if ( !blconf_channel_set_bool (channel, PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, val) )
    {
	g_critical ("Unable to set value for property %s\n", LOCK_SCREEN_ON_SLEEP);
    }
...
/* Light Locker Integration */
void
blpm_update_logind_handle_lid_switch (BlconfChannel *channel)
{
    gboolean lock_on_suspend = blconf_channel_get_bool (channel, PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, TRUE);
    guint lid_switch_on_ac = blconf_channel_get_uint (channel, PROPERTIES_PREFIX LID_SWITCH_ON_AC_CFG, LID_TRIGGER_LOCK_SCREEN);
    guint lid_switch_on_battery = blconf_channel_get_uint (channel, PROPERTIES_PREFIX LID_SWITCH_ON_BATTERY_CFG, LID_TRIGGER_LOCK_SCREEN);

    // logind-handle-lid-switch = true when: lock_on_suspend == true and (lid_switch_on_ac == suspend or lid_switch_on_battery == suspend)
    blconf_channel_set_bool (channel, PROPERTIES_PREFIX LOGIND_HANDLE_LID_SWITCH, lock_on_suspend && (lid_switch_on_ac == 1 || lid_switch_on_battery == 1));
}
/* END Light Locker Integration */

static void
blpm_settings_on_battery (BlconfChannel *channel, gboolean auth_suspend,
                          ...
                          gboolean has_lcd_brightness, gboolean has_lid)
{
    ...
    val = blconf_channel_get_uint (channel,
    ...
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX ON_BATTERY_INACTIVITY_TIMEOUT, 14);
    ...
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX CRITICAL_BATT_ACTION_CFG, XFPM_DO_NOTHING);
     ...
     */
    handle_dpms = blconf_channel_get_bool (channel, PROPERTIES_PREFIX DPMS_ENABLED_CFG, TRUE);

    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX ON_BATT_DPMS_SLEEP, 5);
    gtk_range_set_value (GTK_RANGE(on_battery_dpms_sleep), val);
    gtk_widget_set_sensitive (on_battery_dpms_sleep, handle_dpms);
    
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX ON_BATT_DPMS_OFF, 10);
	...
	val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX LID_SWITCH_ON_BATTERY_CFG, LID_TRIGGER_LOCK_SCREEN);
    ...
    {
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_ON_BATTERY, 120);
    gtk_range_set_value (GTK_RANGE(brg), val);

    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_BATTERY, 20);
...
static void
blpm_settings_on_ac (BlconfChannel *channel, gboolean auth_suspend,
                     gboolean auth_hibernate, gboolean can_suspend,
                     ...
                     gboolean has_lid)
{
    ...
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX ON_AC_INACTIVITY_TIMEOUT, 14);
    gtk_range_set_value (GTK_RANGE (inact_timeout), val);
    ...
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX ON_AC_DPMS_SLEEP, 10);
    gtk_range_set_value (GTK_RANGE (on_ac_dpms_sleep), val);
    gtk_widget_set_sensitive (on_ac_dpms_sleep, handle_dpms);
    
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX ON_AC_DPMS_OFF, 15);
	...
	val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX LID_SWITCH_ON_AC_CFG, LID_TRIGGER_LOCK_SCREEN);
	for ( valid = gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter);
	      valid;
	      valid = gtk_tree_model_iter_next (GTK_TREE_MODEL (list_store), &iter) )
	{
	...
	if ( has_lcd_brightness )
	{
	val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_ON_AC, 9);
	gtk_range_set_value (GTK_RANGE(brg), val);

	val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_AC, 80);
	gtk_range_set_value (GTK_RANGE (brg_level), val);
...
static void
blpm_settings_general (BlconfChannel *channel, gboolean auth_suspend,
                       gboolean auth_hibernate, gboolean can_suspend,
                       gboolean can_hibernate, gboolean can_shutdown,
                       ...
                       gboolean has_power_button)
{
     ...
     */
    val = blconf_channel_get_bool (channel, PROPERTIES_PREFIX DPMS_ENABLED_CFG, TRUE);
	...
	value = blconf_channel_get_uint (channel, PROPERTIES_PREFIX POWER_SWITCH_CFG, XFPM_DO_NOTHING);
	for ( valid = gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter);
	      valid;
	      valid = gtk_tree_model_iter_next (GTK_TREE_MODEL (list_store), &iter) )
	{
	...
	value = blconf_channel_get_uint (channel, PROPERTIES_PREFIX HIBERNATE_SWITCH_CFG, XFPM_DO_NOTHING);
	...
	value = blconf_channel_get_uint (channel, PROPERTIES_PREFIX SLEEP_SWITCH_CFG, XFPM_DO_NOTHING);
	for ( valid = gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter);
	      valid;
	      valid = gtk_tree_model_iter_next (GTK_TREE_MODEL (list_store), &iter) )
	{
    ...
    val = blconf_channel_get_bool (channel, PROPERTIES_PREFIX GENERAL_NOTIFICATION_CFG, TRUE);
    ...
    systray_val = blconf_channel_get_int (channel, PROPERTIES_PREFIX SHOW_TRAY_ICON_CFG, FALSE);

    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(systray), systray_val);
}

static void
blpm_settings_advanced (BlconfChannel *channel, gboolean auth_suspend,
                        gboolean auth_hibernate, gboolean can_suspend,
                        gboolean can_hibernate, gboolean has_battery)
{
	...
	val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX CRITICAL_POWER_LEVEL, 10);
    ...
    val = blconf_channel_get_bool (channel, PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, TRUE);
    ...
    val = blconf_channel_get_bool (channel, PROPERTIES_PREFIX HANDLE_BRIGHTNESS_KEYS, TRUE);
...
void
light_locker_late_locking_value_changed_cb (GtkWidget *widget, BlconfChannel *channel)
{
    GVariant *variant;
    gint      value = (gint)gtk_range_get_value (GTK_RANGE (widget));
...
void
light_locker_automatic_locking_changed_cb (GtkWidget *widget, BlconfChannel *channel)
{
    GVariant *variant;
    gint      value;
...
static void blpm_settings_light_locker (BlconfChannel *channel,
                                        ...
                                        gboolean can_suspend, gboolean can_hibernate)
{
    ...
    GVariant              *variant;
    gboolean               late_locking, lock_on_suspend, blpm_lock_on_suspend;
    ...
    if (schema != NULL && get_light_locker_path() != NULL) {
        ...
        blpm_lock_on_suspend = blconf_channel_get_bool (channel, PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, TRUE);
        if (lock_on_suspend != blpm_lock_on_suspend) {
            variant = g_variant_new_boolean (blpm_lock_on_suspend);
            if (!g_settings_set_value (light_locker_settings, "lock-on-suspend", variant)) {
                g_critical ("Cannot set value for property lock-on-suspend\n");
            }
            lock_on_suspend = blpm_lock_on_suspend;
        }
...
static gchar *
blpm_info_get_energy_property (gdouble energy, const gchar *unit)
{
    gchar *val = NULL;
...
update_device_details (UpDevice *device)
{
    ...
    {
        battery_type = blpm_power_translate_device_type (type);
    ...
    if ( type != UP_DEVICE_KIND_LINE_POWER )
    {
        ...
        update_device_info_value_for_name (view, list_store, _("Technology"), blpm_power_translate_technology (tech));
        ...
        if (energy_full_design > 0)
        {
            ...
            str = blpm_info_get_energy_property (energy_full_design, _("Wh"));
        ...
        if (energy_full > 0)
        {
            ...
            /* TRANSLATORS: Unit here is Watt hour*/
            str = blpm_info_get_energy_property (energy_full, _("Wh"));
        ...
        {
            /* TRANSLATORS: Unit here is Watt hour*/
            str = blpm_info_get_energy_property (energy_empty, _("Wh"));
        ...
        if (voltage > 0)
        {
            ...
            str = blpm_info_get_energy_property (voltage, _("V"));
...
static void
settings_quit (GtkWidget *widget, BlconfChannel *channel)
{
...
static void dialog_response_cb (GtkDialog *dialog, gint response, BlconfChannel *channel)
{
    switch(response)
    {
	case GTK_RESPONSE_HELP:
#if LIBBLADEUI_CHECK_VERSION(4, 11, 1)
	    xfce_dialog_show_help_with_version (NULL, "blade-pm", "start", NULL, XFPM_VERSION_SHORT);
#else
	    xfce_dialog_show_help (NULL, "blade-pm", "start", NULL);
#endif
	    break;
	default:
...
static void
delete_event_cb (GtkWidget *plug, GdkEvent *ev, BlconfChannel *channel)
{
    settings_quit (plug, channel);
}

GtkWidget *
blpm_settings_dialog_new (BlconfChannel *channel, gboolean auth_suspend,
                          gboolean auth_hibernate, gboolean can_suspend,
                          gboolean can_hibernate, gboolean can_shutdown,
                          ...
                          GdkNativeWindow id, gchar *device_id)
{
    ...
    XFPM_DEBUG ("auth_hibernate=%s auth_suspend=%s can_shutdown=%s can_suspend=%s can_hibernate=%s " \
      ...
      blpm_bool_to_string (has_battery), blpm_bool_to_string (auth_hibernate),
	  blpm_bool_to_string (can_shutdown), blpm_bool_to_string (auth_suspend),
	  blpm_bool_to_string (can_suspend), blpm_bool_to_string (can_hibernate),
	  blpm_bool_to_string (has_lcd_brightness), blpm_bool_to_string (has_lid),
	  blpm_bool_to_string (has_sleep_button), blpm_bool_to_string (has_hibernate_button),
	  blpm_bool_to_string (has_power_button));

    xml = blpm_builder_new_from_string (blpm_settings_ui, &error);
    ...
    val = blconf_channel_get_uint (channel, PROPERTIES_PREFIX ON_BATTERY_BLANK, 1);
    gtk_range_set_value (GTK_RANGE (on_battery_display_blank), val);
    blconf_g_property_bind (channel, PROPERTIES_PREFIX ON_BATTERY_BLANK,
    ...
    blconf_g_property_bind (channel, PROPERTIES_PREFIX ON_AC_BLANK,
                            G_TYPE_INT, gtk_range_get_adjustment (GTK_RANGE (on_ac_display_blank)),
                            "value");

    dialog = GTK_WIDGET (gtk_builder_get_object (xml, "blpm-settings-dialog"));
    ...
    if ( has_battery )
    blpm_settings_on_battery (channel,
                              auth_suspend,
                              auth_hibernate,
                              can_suspend,
                              can_hibernate,
    ...
    blpm_settings_general (channel, auth_suspend, auth_hibernate, can_suspend, can_hibernate, can_shutdown,
                           has_sleep_button, has_hibernate_button, has_power_button );

    blpm_settings_advanced (channel, auth_suspend, auth_hibernate, can_suspend, can_hibernate, has_battery);

    /* Light Locker Integration */
    blpm_settings_light_locker (channel, auth_suspend, auth_hibernate, can_suspend, can_hibernate);
    /* END Light Locker Integration */

...
static void blpm_kbd_backlight_finalize     (GObject *object);
...
G_DEFINE_TYPE (XfpmKbdBacklight, blpm_kbd_backlight, G_TYPE_OBJECT)
...
static void
blpm_kbd_backlight_on_battery_changed_cb (XfpmPower *power, gboolean on_battery, XfpmKbdBacklight *backlight)
{
...
static void
blpm_kbd_backlight_init_max_level (XfpmKbdBacklight *backlight)
{
    GError *error = NULL;
...
static void
blpm_kbd_backlight_show_notification (XfpmKbdBacklight *self, gfloat value)
{
    gchar *summary;

    if ( self->priv->n == NULL )
    {
        self->priv->n = blpm_notify_new_notification (self->priv->notify,
                "",
                "",
                "blpm-brightness-keyboard",
                0,
                XFPM_NOTIFY_NORMAL,
                NULL);
    }
...
static gint
blpm_kbd_backlight_get_level (XfpmKbdBacklight *backlight)
{
    GError *error = NULL;
    gint level = -1;
...
static void
blpm_kbd_backlight_set_level (XfpmKbdBacklight *backlight, gint level)
{
    GError *error = NULL;
    gfloat percent;
    ...
    else
    {
        ...
        blpm_kbd_backlight_show_notification (backlight, percent);
    }
}

static void
blpm_kbd_backlight_up (XfpmKbdBacklight *backlight)
{
    gint level;

    level = blpm_kbd_backlight_get_level(backlight);
    ...
    blpm_kbd_backlight_set_level(backlight, level);
}


static void
blpm_kbd_backlight_down (XfpmKbdBacklight *backlight)
{
    gint level;
...
static void
blpm_kbd_backlight_button_pressed_cb (XfpmButton *button, XfpmButtonKey type, XfpmKbdBacklight *backlight)
{
    if ( type == BUTTON_KBD_BRIGHTNESS_UP )
    {
        blpm_kbd_backlight_up (backlight);
    }
    else if ( type == BUTTON_KBD_BRIGHTNESS_DOWN )
    {
        blpm_kbd_backlight_down (backlight);
    }
}


static void
blpm_kbd_backlight_class_init (XfpmKbdBacklightClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->finalize = blpm_kbd_backlight_finalize;
...
static void
blpm_kbd_backlight_init (XfpmKbdBacklight *backlight)
{
    GError *error = NULL;
    ...
    blpm_kbd_backlight_init_max_level (backlight);
    ...
    g_signal_connect (backlight->priv->button, "button-pressed",
                      G_CALLBACK (blpm_kbd_backlight_button_pressed_cb), backlight);

    g_signal_connect (backlight->priv->power, "on-battery-changed",
                      G_CALLBACK (blpm_kbd_backlight_on_battery_changed_cb), backlight);
...
static void
blpm_kbd_backlight_finalize (GObject *object)
{
    ...
    G_OBJECT_CLASS (blpm_kbd_backlight_parent_class)->finalize (object);
}


XfpmKbdBacklight *
blpm_kbd_backlight_new (void)
{
    XfpmKbdBacklight *backlight = NULL;
...
gboolean blpm_kbd_backlight_has_hw (XfpmKbdBacklight *backlight)
{

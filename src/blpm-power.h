...
#define XFPM_TYPE_POWER        (blpm_power_get_type () )
...
GType        		blpm_power_get_type        	(void) G_GNUC_CONST;

XfpmPower       	       *blpm_power_get             	(void);

void			blpm_power_suspend         	(XfpmPower *power,
							 gboolean force);

void			blpm_power_hibernate       	(XfpmPower *power,
							 gboolean force);

gboolean		blpm_power_has_battery		(XfpmPower *power);

gboolean        blpm_power_is_in_presentation_mode (XfpmPower *power);

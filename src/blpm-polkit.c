...
static void blpm_polkit_finalize   (GObject *object);
...
G_DEFINE_TYPE (XfpmPolkit, blpm_polkit, G_TYPE_OBJECT)
...
#ifdef ENABLE_POLKIT
static gboolean
blpm_polkit_free_data (gpointer data)
{
    XfpmPolkit *polkit;
...
static void
blpm_polkit_init_data (XfpmPolkit *polkit)
{
    GValue hash_elem = { 0 };
    gint pid;
    guint64 start_time;
	...
	g_timeout_add_seconds (60, (GSourceFunc) blpm_polkit_free_data, polkit);
...
static gboolean
blpm_polkit_check_auth_intern (XfpmPolkit *polkit, const gchar *action_id)
{
#ifdef ENABLE_POLKIT
    GValueArray *result;
    GValue result_val = { 0 };
    ...
    XFPM_DEBUG ("Action=%s is authorized=%s", action_id, blpm_bool_to_string (is_authorized));
...
#ifdef ENABLE_POLKIT
static void
blpm_polkit_changed_cb (DBusGProxy *proxy, XfpmPolkit *polkit)
{
...
static void
blpm_polkit_class_init (XfpmPolkitClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->finalize = blpm_polkit_finalize;
...
static void
blpm_polkit_init (XfpmPolkit *polkit)
{
    GError *error = NULL;
	...
	dbus_g_proxy_connect_signal (polkit->priv->proxy, "Changed",
				     G_CALLBACK (blpm_polkit_changed_cb), polkit, NULL);
    }
    else
    {
...
static void
blpm_polkit_finalize (GObject *object)
{
    XfpmPolkit *polkit;
	...
	dbus_g_proxy_disconnect_signal (polkit->priv->proxy, "Changed",
					G_CALLBACK (blpm_polkit_changed_cb), polkit);
	g_object_unref (polkit->priv->proxy);
    }

    if ( polkit->priv->subject_valid )
    {
	blpm_polkit_free_data (polkit);
    ...
    G_OBJECT_CLASS (blpm_polkit_parent_class)->finalize (object);
}

XfpmPolkit *
blpm_polkit_get (void)
{
    static gpointer blpm_polkit_obj = NULL;
    
    if ( G_LIKELY (blpm_polkit_obj) )
    {
	g_object_ref (blpm_polkit_obj);
    }
    else
    {
	blpm_polkit_obj = g_object_new (XFPM_TYPE_POLKIT, NULL);
	g_object_add_weak_pointer (blpm_polkit_obj, &blpm_polkit_obj);
    }
    
    return XFPM_POLKIT (blpm_polkit_obj);
}

gboolean blpm_polkit_check_auth	(XfpmPolkit *polkit, const gchar *action_id)
{
#ifdef ENABLE_POLKIT
    blpm_polkit_init_data (polkit);
#endif
    return blpm_polkit_check_auth_intern (polkit, action_id);
}

...
static void blpm_systemd_finalize   (GObject *object);

static void blpm_systemd_get_property (GObject *object,
                                       guint prop_id,
                                       GValue *value,
                                       GParamSpec *pspec);
...
G_DEFINE_TYPE (XfpmSystemd, blpm_systemd, G_TYPE_OBJECT)
...
static void
blpm_systemd_class_init (XfpmSystemdClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->finalize = blpm_systemd_finalize;

    object_class->get_property = blpm_systemd_get_property;
...
static gboolean
blpm_systemd_can_method (XfpmSystemd  *systemd,
                         gboolean     *can_method,
                         const gchar  *method)
{
    *can_method = FALSE;
...
#ifdef ENABLE_POLKIT
    *can_method = blpm_polkit_check_auth(systemd->priv->polkit, method);
...
static void
blpm_systemd_finalize (GObject *object)
{
#ifdef ENABLE_POLKIT
    XfpmSystemd *systemd;
    ...
    G_OBJECT_CLASS (blpm_systemd_parent_class)->finalize (object);
}

XfpmSystemd *
blpm_systemd_new (void)
{
...
static void
blpm_systemd_try_method (XfpmSystemd  *systemd,
                         const gchar  *method,
                         GError      **error)
{
    GDBusConnection *bus;
...
void blpm_systemd_shutdown (XfpmSystemd *systemd, GError **error)
{
    blpm_systemd_try_method (systemd,
                             SYSTEMD_POWEROFF_ACTION,
                             error);
}

void blpm_systemd_reboot (XfpmSystemd *systemd, GError **error)
{
    blpm_systemd_try_method (systemd,
...
void blpm_systemd_sleep (XfpmSystemd *systemd,
                         const gchar *method,
                         GError **error)
{
    blpm_systemd_try_method (systemd, method, error);
}

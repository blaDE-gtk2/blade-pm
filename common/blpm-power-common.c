...
/**
 * blpm_power_translate_device_type:
 *
 **/
const gchar *
blpm_power_translate_device_type (guint type)
{
    switch (type)
    {
...
/**
 * blpm_power_translate_technology:
 *
 **/
const gchar *
blpm_power_translate_technology (guint value)
{
    switch (value)
    {
        case 0:
...
blpm_battery_get_icon_index (UpDeviceKind type, guint percent)
{
    if (percent < 10)
    {
        return "000";
 ...
 */
gchar *
blpm_battery_get_time_string (guint seconds)
{
...
gchar *
blpm_battery_get_icon_prefix_device_enum_type (UpDeviceKind type)
{
    /* mapped from
...
get_device_icon_name (UpClient *upower, UpDevice *device)
{
    ...
    icon_prefix = blpm_battery_get_icon_prefix_device_enum_type (type);
	...
	{
	    icon_name = g_strdup_printf ("%s%s-%s", icon_prefix, blpm_battery_get_icon_index (type, percentage), "charging");
	}
	else if ( state == UP_DEVICE_STATE_DISCHARGING || state == UP_DEVICE_STATE_PENDING_DISCHARGE)
	{
	    icon_name = g_strdup_printf ("%s%s", icon_prefix, blpm_battery_get_icon_index (type, percentage));
	}
...
get_device_description (UpClient *upower, UpDevice *device)
{
    ...
    if (g_strcmp0(vendor, "") == 0 && g_strcmp0(model, "") == 0)
        vendor = g_strdup_printf ("%s", blpm_power_translate_device_type (type));
    ...
    if ( state == UP_DEVICE_STATE_FULLY_CHARGED )
    {
        if ( time_to_empty > 0 )
        {
            est_time_str = blpm_battery_get_time_string (time_to_empty);
    ...
    else if ( state == UP_DEVICE_STATE_CHARGING )
    {
        if ( time_to_full != 0 )
        {
            est_time_str = blpm_battery_get_time_string (time_to_full);
            tip = g_strdup_printf (_("<b>%s %s</b>\nCharging (%0.0f%%, %s)"),
                                   vendor, model,
                                   percentage,
                                   est_time_str);
